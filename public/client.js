...
function renderLobby(s) {
  ...
  $('#lobby-count').textContent = `${s.players.length} / 7`;
  const ul = $('#lobby-players'); ul.innerHTML = '';
  s.players.forEach(p => {
    const li = document.createElement('li');
    li.innerHTML = `
      <div class="avatar">${escapeHtml(initial(p.name))}</div>
      <span class="p-name">${escapeHtml(p.name)}${p.isYou ? '（你）' : ''}</span>
      <span class="tag ${p.isHost ? 'host' : ''} ${p.ready ? 'ready' : ''}">${p.isHost ? '房主' : (p.ready ? '已准备' : '未准备')}</span>`;
    ul.appendChild(li);
  });
  ...
  if (s.isHost) {
    ...
    if (sel.options.length === 0) {
      PRESETS_CACHE.forEach(p => { const o = document.createElement('option'); o.value = p.id; o.textContent = p.name; sel.appendChild(o); });
    }
    sel.value = s.presetId;
    const pr = PRESETS_CACHE.find(p => p.id === s.presetId);
    $('#preset-desc').innerHTML = pr ? '本局身份牌：<b>' + pr.cards.join('、') + '</b>' : '';
  }
}
...
$('#preset-select').onchange = (e) => api('/api/action', { token: STATE.token, type: 'setPreset', payload: { presetId: e.target.value } });
