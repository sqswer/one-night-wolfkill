...
const ROLES = {
  ...
  guardian:            { name: '监护人',   team: 'village', nightOrder: 99, wake: true,  action: 'guardian',   hint: '监护人请睁眼，守护一名玩家。' },
...
// ----------------------------- 预设阵容（来自 5人平衡阵容指南） -----------------------------
// 每张预设固定 8 张身份牌：发玩家人 + 余下置中央底牌。
const PRESETS = [
  { id: 'M1', name: 'M1·新手友好·明察(首推)', cards: ['werewolf','minion','seer','robber','troublemaker','insomniac','mason','mason'], hasDusk: false },
  { id: 'M2', name: 'M2·双预言·信息局',       cards: ['werewolf','minion','seer','apprentice_seer','robber','insomniac','troublemaker','villager'], hasDusk: false },
  { id: 'M3', name: 'M3·女巫搅局(破晓1)',     cards: ['werewolf','minion','seer','witch','robber','troublemaker','insomniac','sentinel'], hasDusk: false },
  { id: 'M4', name: 'M4·灵异侦探局(破晓1)',   cards: ['werewolf','minion','seer','paranormal_detective','witch','robber','insomniac','villager'], hasDusk: false },
  { id: 'M5', name: 'M5·保镖守护(破晓1)',     cards: ['werewolf','minion','seer','robber','troublemaker','drunk','insomniac','bodyguard'], hasDusk: false },
  { id: 'M6', name: 'M6·基础纯净局(仅基础)',  cards: ['werewolf','minion','seer','robber','troublemaker','drunk','insomniac','hunter'], hasDusk: false },
  { id: 'A',  name: 'A·化身幽灵 chaos',        cards: ['doppelganger','werewolf','minion','seer','robber','witch','troublemaker','insomniac'], hasDusk: false },
  { id: 'B',  name: 'B·皮匠搅局(三阵营)',      cards: ['werewolf','minion','tanner','seer','robber','troublemaker','drunk','insomniac'], hasDusk: false },
  { id: 'C',  name: 'C·猎人护航',              cards: ['werewolf','minion','hunter','seer','robber','troublemaker','drunk','insomniac'], hasDusk: false },
  { id: 'D',  name: 'D·破晓重度(阿尔法+哨兵)', cards: ['alpha_wolf','werewolf','minion','seer','witch','insomniac','sentinel','revealer'], hasDusk: false },
  { id: 'E',  name: 'E·王子与被诅咒者(破晓2)', cards: ['werewolf','minion','prince','cursed','seer','robber','troublemaker','insomniac'], hasDusk: false },
  { id: 'V1', name: 'V1·吸血鬼入门(简单)',     cards: ['vampire','renfield','seer','robber','troublemaker','insomniac','sentinel','villager'], hasDusk: true },
  { id: 'V2', name: 'V2·伯爵控场(进阶1)',      cards: ['vampire','count','renfield','priest','sharpshooter','robber','troublemaker','sentinel'], hasDusk: true },
  { id: 'V3', name: 'V3·刺客与爱神(进阶2)',    cards: ['vampire','count','renfield','priest','sharpshooter','thief','cupid','assassin'], hasDusk: true },
  { id: 'V4', name: 'V4·标记混战(进阶3)',      cards: ['vampire','count','renfield','priest','sharpshooter','thief','gremlin','drunk'], hasDusk: true },
  { id: 'V5', name: 'V5·狼血大战(史诗)',       cards: ['werewolf','minion','vampire','renfield','priest','sharpshooter','robber','troublemaker'], hasDusk: true },
];
...
function buildState(room, player) {
  ...
  const state = {
    ...
    presetName: PRESETS.find(x => x.id === room.presetId)?.name || '',
    you,
    action,
...
function startGame(room) {
  const preset = PRESETS.find(x => x.id === room.presetId) || PRESETS[0];
  const n = room.players.length;
  if (n < 3 || n > 7) { return { error: '人数需为 3-7 人' }; }
  // 8 张身份牌 = 玩家人 + 中央底牌
  const deck = shuffle(preset.cards.slice());
  room.deck = deck;
  room.centerCount = 8 - n;
  ...
  publicLog(room, `游戏开始 · 阵容【${preset.name}】· 共 ${n} 人，中央 ${room.centerCount} 张底牌。`);
  announce(room, `游戏开始。阵容：${preset.name}。共 ${n} 位玩家，中央放置 ${room.centerCount} 张底牌。请各位查看自己的身份。`);
...
function advanceQueue(room) {
  ...
  if (needsInput) {
    ...
    pushState(room);
    const delay = room.phase === 'lobby' ? 1000 : 4200;
...
function roleNeedsInput(role, room, seats) {
  ...
  const inputRoles = {
    ...
    wolfSeer: true, doppelganger: true, guardian: false, revealer: false,
...
function roleReveal(role, room, seats) {
  ...
  } else if (role === 'guardian' || role === 'revealer') {
...
function handleNightAction(room, token, payload) {
  ...
  applyAction(room, action, action.submissions);
  announce(room, `请【${ROLES[action.role].name}】闭眼。`);
  room.currentAction = null;
  pushState(room);
  room.nightTimer = setTimeout(() => advanceQueue(room), 1400);
  return { ok: true };
}
...
function startVote(room) {
  ...
  // 保镖保护行动（若有保镖）
  const bodyguards = room.players.filter(p => room.currentRole[p.seat] === 'bodyguard').map(p => p.seat);
  if (bodyguards.length) {
    room.currentAction = { role: 'bodyguard', seats: bodyguards, submissions: {} };
...
function computeResult(room, outSet) {
  ...
  const perPlayer = room.players.map(p => {
    ...
    return { seat: p.seat, name: p.name, role: rk, roleName: r.name, team, out: outSeatSet.has(p.seat), win };
  });
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === 'GET' && pathname === '/api/presets') {
    return sendJSON(res, PRESETS.map(p => ({ id: p.id, name: p.name, cards: p.cards.map(c => ROLES[c].name), hasDusk: p.hasDusk })));
  }
  ...
  if (req.method === 'POST' && pathname === '/api/create') {
    ...
    req.on('end', () => {
      let name = '玩家'; try { name = (JSON.parse(body).name || '玩家').toString().slice(0, 16); } catch (_) {}
  ...
  if (req.method === 'POST' && pathname === '/api/join') {
    ...
    req.on('end', () => {
      ...
      if (room.players.length >= 7) return sendJSON(res, { error: '房间已满（最多7人）' }, 409);
      const token = genToken();
    ...
    req.on('end', () => {
      ...
      switch (type) {
        case 'setPreset':
          ...
          if (PRESETS.find(x => x.id === payload.presetId)) { room.presetId = payload.presetId; pushState(room); }
          break;
        case 'ready':
